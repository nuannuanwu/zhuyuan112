...
from memory.helper import verify_sms_helper

def pwd_back_mail(request, template_name="memory/pwd_back/pwd_back_mail_a.html"):
    """
    通过邮箱找回密码页
    """
    ...
    post_reset_redirect = reverse('memory_pwd_back_mail_done')

    return auth_views.password_reset(request, template_name="memory/pwd_back/pwd_back_mail_a.html",\
     email_template_name='memory/pwd_back/password_reset_message.txt',post_reset_redirect=post_reset_redirect,\
     extra_context={'mail':True})

def pwd_back_mail_done(request, template_name="memory/pwd_back/pwd_back_mail_done.html"):
    """
    邮箱成功页
    """
...
def pwd_back_mail_reset(request,uidb36=None,token=None, \
    template_name='memory/pwd_back/pwd_back_reset_mail.html'):
    """
    邮箱密码重置页
    """
    post_reset_redirect = reverse('memory_pwd_back_success') + "?type=mail"
...
def pwd_back_mobile(request, template_name="memory/pwd_back/pwd_back_mobile_a.html"):
    """
    通过手机找回密码处理页面，单击下一步
    """
    print 2222
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('memory_pwd_back_pwd_reset')
...
def pwd_back_pwd_reset(request, template_name="memory/pwd_back/pwd_back_reset.html"):
    """
    密码重置，mobile
    """
    ...
    if not s:
        return redirect('memory_pwd_back_mobile')
    ...
    if request.method == 'POST': 
        ...
        if form.is_valid():
            ...
            url = reverse('memory_pwd_back_success') + "?type=mobile"
...
def pwd_back_success(request, template_name="memory/pwd_back/pwd_back_success.html"):
    """
